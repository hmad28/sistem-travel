...
export default async function Page() {
  ...
  return <PageShell title={t('registrationsTitle')} description={t('flow')} actions={<Link href="/admin/manajemen/pendaftaran/baru" className="inline-flex min-h-12 items-center rounded-lg bg-primary px-5 text-primary-foreground">{t('register')}</Link>}><SimpleRecords title={t('registrationsTitle')} headers={[t('registrationNumber'),t('pilgrimId'),t('packageId'),t('departureDate'),t('startingPrice')]} rows={rows.map(({r,p,d,pkg})=>[r.registrationNumber,p.fullName,pkg.name,d.departureDate,formatIdr.format(r.finalPrice)])} /></PageShell>;
}
